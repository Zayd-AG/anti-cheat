# Anti-Cheat / Trust & Safety Pipeline

An educational backend portfolio project that ingests simulated game telemetry, detects suspicious sessions, and exposes a cached trust score. It progresses from a transparent statistical baseline to an async, ML-backed service.
...
FastAPI validates and accepts telemetry quickly. It does **not** run model inference during the request: scoring happens in Celery so bursts do not exhaust web workers, worker capacity can scale independently, and transient Redis failures can retry safely. Redis is the Celery broker and the short-lived lookup cache; those are separate uses of the same service.
...
The data is synthetic and intentionally makes cheating distinct. Real cheaters are more varied and adversarial, while legitimate players differ by device, accessibility needs, connection quality, skill level, and game mode. A production service would need privacy controls, calibrated thresholds, review workflows, drift monitoring, and carefully evaluated appeals—not automatic punitive decisions from this score alone.
